...
class SaveFileNintendo64 extends SaveFile {
	...
	async swapWords(bytes = 2) {
		...
		for(let i = 0; i < this.buffer.length; i += (bytes * 2) ) {
			...
			let tempArray = this.buffer.slice(FIRST_BYTE_OFFSET_START, FIRST_BYTE_OFFSET_END + 1); // add +1 to be inclusive of last byte
			let tempArray2 = this.buffer.slice(SECOND_BYTE_OFFSET_START,SECOND_BYTE_OFFSET_END + 1); // add +1 to be inclusive of last byte
